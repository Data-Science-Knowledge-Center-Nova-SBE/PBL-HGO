...
def unit(alertP1):
    ...
    alertP1['unit']=['USF' if x in usf_A else 'USF' if x in usf_B else 'UCSP' if x in UCSP_list else 'CS' if x in CS else 'outro' if x in outro else 'HGO' if x==0 else 'HOSP' for x in alertP1['COD_UNID_SAUDE_PROV']]
